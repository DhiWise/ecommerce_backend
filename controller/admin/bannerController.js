...
const addBanner = async (req, res) => {
  try {
    ...
    let result = await dbService.createDocument(Banner,data);
...
const findAllBanner = async (req,res) => {
  try {
    ...
    if (req.body.isCountOnly){
      result = await dbService.countDocument(Banner, query);
      if (result) {
      ...
      result = await dbService.getAllDocuments( Banner,query,options);
...
const getBanner = async (req,res) => {
  try {
    ...
    let result = await dbService.getDocumentByQuery(Banner,query);
    if (result){
      return  res.ok(result);
...
const getBannerCount = async (req,res) => {
  try {
    ...
    let result = await dbService.countDocument(Banner,where);
    if (result){
      result = { totalRecords:result };
      return res.ok(result);
    }
...
const getBannerByAggregate = async (req,res)=>{
  try {
    let result = await dbService.getDocumentByAggregation(Banner,req.body);
    if (result){
      return res.ok(result);
    }
    return res.recordNotFound({});
...
const updateBanner = async (req,res) => {
  try {
    ...
    let result = await dbService.findOneAndUpdateDocument(Banner,query,data,{ new:true });
    if (!result){
...
const softDeleteBanner = async (req,res) => {
  try {
    let query = { _id:req.params.id };
    let result = await dbService.findOneAndUpdateDocument(Banner, query,{
      isDeleted: true,
      updatedBy:req.user.id
    },{ new:true });
    if (!result){
...
const softDeleteManyBanner = async (req,res) => {
  try {
    ...
    if (ids){
      ...
      let data = await dbService.bulkUpdate(Banner,query, {
        isDeleted: true,
        updatedBy:req.user.id
      });
      if (!data) {
...
const bulkInsertBanner = async (req,res)=>{
  try {
    ...
    if (req.body.data !== undefined && req.body.data.length){
      ...
      let result = await dbService.bulkInsert(Banner,data);
      return  res.ok(result);
    } else {
...
const bulkUpdateBanner = async (req,res)=>{
  try {
    ...
    if (req.body.data !== undefined){
      ...
      let result = await dbService.bulkUpdate(Banner,filter,data);
      if (!result){
        return res.failureResponse('something is wrong.');
      }
