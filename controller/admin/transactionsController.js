...
const addTransactions = async (req, res) => {
  try {
    ...
    let result = await dbService.createDocument(Transactions,data);
...
const findAllTransactions = async (req,res) => {
  try {
    ...
    if (req.body.isCountOnly){
      result = await dbService.countDocument(Transactions, query);
      if (result) {
      ...
      result = await dbService.getAllDocuments( Transactions,query,options);
...
const getTransactions = async (req,res) => {
  try {
    ...
    let result = await dbService.getDocumentByQuery(Transactions,query);
    if (result){
      return  res.ok(result);
...
const getTransactionsCount = async (req,res) => {
  try {
    ...
    let result = await dbService.countDocument(Transactions,where);
    if (result){
      result = { totalRecords:result };
      return res.ok(result);
    }
...
const getTransactionsByAggregate = async (req,res)=>{
  try {
    let result = await dbService.getDocumentByAggregation(Transactions,req.body);
    if (result){
      return res.ok(result);
    }
    return res.recordNotFound({});
...
const updateTransactions = async (req,res) => {
  try {
    ...
    let result = await dbService.findOneAndUpdateDocument(Transactions,query,data,{ new:true });
    if (!result){
...
const softDeleteTransactions = async (req,res) => {
  try {
    let query = { _id:req.params.id };
    let result = await dbService.findOneAndUpdateDocument(Transactions, query,{
      isDeleted: true,
      updatedBy:req.user.id
    },{ new:true });
    if (!result){
...
const softDeleteManyTransactions = async (req,res) => {
  try {
    ...
    if (ids){
      ...
      let data = await dbService.bulkUpdate(Transactions,query, {
        isDeleted: true,
        updatedBy:req.user.id
      });
      if (!data) {
...
const bulkInsertTransactions = async (req,res)=>{
  try {
    ...
    if (req.body.data !== undefined && req.body.data.length){
      ...
      let result = await dbService.bulkInsert(Transactions,data);
      return  res.ok(result);
    } else {
...
const bulkUpdateTransactions = async (req,res)=>{
  try {
    ...
    if (req.body.data !== undefined){
      ...
      let result = await dbService.bulkUpdate(Transactions,filter,data);
      if (!result){
        return res.failureResponse('something is wrong.');
      }
